...
func (srv *Server) Start() (err error) {
	...
	// variables a node need to store
	/*
	 int level;            // the level of the ring this node is at
	 bool has_subring;     // whether it's a contact node of a sub-ring
	 string role;          // "normal" "contact"
	 curr_contact_nodes[]  // the contact nodes of this ring
	 upper_contact_nodes[] // the contact nodes of the upper level ring
	 node predecessor      // the one behind this node on the ring
	 node successor        // the one next to this node on the ring)
	 node pre-predecessor  // used for checking liveness of its predecessor)
	 node suc-successor    // used for checking liveness of its successor)
	 int sequence_number   // int, deferenciate which broadcast, for each broadcast there will be a new contact node or a few contact nodes)
	 int broadcast_key     // k-nary broadcast, randomly generated by the contact node (2, 3, 4, 5)
	*/
