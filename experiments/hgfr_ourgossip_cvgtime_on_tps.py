...
GROUPE = ['Gossip with 6K nodes', 'HGFRR with 6K nodes', 'Gossip with 2K nodes', 'HGFRR with 2K nodes']
...
for file, color in zip(FILES, COLORS):
    ...
    plt.plot(x, y_eth2k[file], marker='o', markersize=6, linewidth=2, label='Gossip with 2K nodes', color=color)
    plt.plot(x, y_our2k[file], marker='D', markersize=6, linewidth=2, label='HGFRR with 2K nodes', color=color)
...
lgd=ax.legend(handles=HANDLES, fontsize=12, prop={'weight':'bold'}, loc=9, bbox_to_anchor=(0.45, -0.1), ncol=4)
...
plt.savefig('hgfr_ourgossip_cvgtime_on_tps.pdf')
plt.show()