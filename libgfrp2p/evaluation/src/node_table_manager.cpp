...
/* Constructor */
NodeTableManager::NodeTableManager(const std::shared_ptr<NodeTable>& node_table, unsigned short port):
    udp_server(new AsyncUDPServer(shared_from_this(), port)), node_table(node_table) { }

/* Private functions */
void NodeTableManager::receive(const std::string& ip, unsigned short port, const std::string& datagram) {
...
void NodeTableManager::send(const std::string& ip, unsigned short port, std::string payload) {
    Packet packet;
    packet.set_payload(payload);
    this->udp_server->send(ip, port, packet.pack());
}

void NodeTableManager::send_ping(const std::string& id, const std::string& ip, unsigned short port) {
...
void NodeTableManager::send_pong(const std::string& ip, unsigned short port) {
    std::string payload = this->node_table->get_self_id() + CMD_PONG;
    this->send(ip, port, payload);
}

void NodeTableManager::receive_ping(const std::string& ip, unsigned short port) {
    send_pong(ip, port);
}

void NodeTableManager::receive_pong(const std::string& id, const std::string& ip, unsigned short port) {
    if (this->node_table->has_node(id)) {
...
void NodeTableManager::start() {
    this->udp_server->run();
}

void NodeTableManager::stop() {
